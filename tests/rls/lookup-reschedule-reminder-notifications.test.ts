...
const countAlarms = (ics: string) =>
  (ics.match(/BEGIN:VALARM/g) ?? []).length;
...
async function layerA() {
  ...
  await test(
    "DTSTART reflects the NEW date/time after reschedule (not the original)",
    () => {
      const original = buildIcs(
        base({ appointment_date: "2099-01-15", appointment_time: "10:00" }),
      );
      const rescheduled = buildIcs(
        base({ appointment_date: "2099-02-20", appointment_time: "14:30" }),
      );
      assert(
        original.includes(expectedDtstart("2099-01-15", "10:00")),
        "original DTSTART wrong",
      );
      assert(
  ...
  await test(
    "UID is stable across reschedule (same ref → calendar clients replace the event)",
    () => {
      const a = buildIcs(
        base({ appointment_date: "2099-01-15", appointment_time: "10:00" }),
      );
  ...
  await test(
    "toggling 24h OFF then ON regenerates VALARM(-PT24H) (cancel → recreate)",
    () => {
...
async function layerB() {
  ...
  async function readRow(id: string) {
    const { data, error } = await admin
      .from("appointments")
      .select(
        "appointment_date, appointment_time, reminder_24h, reminder_2h",
      )
      .eq("id", id)
      .single();
    if (error) throw error;
  ...
  try {
    await test(
      "reschedule keeps both reminders → new ICS has 2 VALARM at new DTSTART",
      async () => {
    ...
    await test(
      "disabling both reminders cancels notifications (ICS has no VALARM)",
      async () => {
        const { id, ref } = await newAppt({ reminder_24h: true, reminder_2h: true });
    ...
    await test(
      "mixed selection (24h on, 2h off) → ICS has only -PT24H",
      async () => {
        const { id, ref } = await newAppt({ reminder_24h: true, reminder_2h: true });
    ...
    await test(
      "previously OFF reminders can be turned back ON (notifications recreated)",
      async () => {
