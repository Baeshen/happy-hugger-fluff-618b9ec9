...
(async () => {
  ...
  try {
    ...
    const cases: Array<{
      label: string; from: "confirmed" | "new"; to: "cancelled" | "no_show" | "completed";
      raw: string; expected: string;
    }> = [
      { label: "cancelled + plain reason", from: "confirmed", to: "cancelled", raw: CORE, expected: CORE },
      { label: "no_show + padded reason (edges trimmed)", from: "confirmed", to: "no_show",
        raw: `  \t\n\u00A0${CORE}\u00A0 \r\n`, expected: CORE },
      { label: "completed + optional reason recorded", from: "confirmed", to: "completed",
        raw: "  تم الحضور والفحص  ", expected: "تم الحضور والفحص" },
      { label: "cancelled + 500-char core with padding", from: "confirmed", to: "cancelled",
        raw: `\n\t \u00A0${"ا".repeat(REASON_MAX)}\u00A0 \r\n`, expected: "ا".repeat(REASON_MAX) },
    ];

    for (const { label, from, to, raw, expected } of cases) {
      await test(label, async () => {
        const a = await newAppt(from); created.push(a.id);

        const { error } = await c.rpc("update_appointment_status" as any, {
          _id: a.id, _status: to, _reason: raw,
        });
        ...
        assert(r.old_notes === null && r.new_notes === null,
        ...
        assert(!/^[\s\u00A0]|[\s\u00A0]$/.test(r.reason as string),
