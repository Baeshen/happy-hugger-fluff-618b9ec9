...
async function runPureTests() {
  ...
  const spaceCases: Array<[string, string, string]> = [
    ...
    ["internal NBSP run kept",      `اتصل${NBSP}${NBSP}${NBSP}المريض`,        `اتصل${NBSP}${NBSP}${NBSP}المريض`],
    ["mixed NBSP + space kept",     `اتصل${NBSP}${SP}${NBSP}المريض`,          `اتصل${NBSP}${SP}${NBSP}المريض`],
    ["NBSP between words, edges trimmed",
      `  اتصل${NBSP}المريض  `,                                                `اتصل${NBSP}المريض`],
    ["edge NBSP + internal NBSP",
      `${NBSP}اتصل${NBSP}${NBSP}المريض${NBSP}`,                               `اتصل${NBSP}${NBSP}المريض`],
    ["tabs stripped at edges, internal tab kept",
      `\tاتصل\tالمريض\t`,                                                     `اتصل\tالمريض`],
...
async function runDbTests() {
  ...
  const cases: Array<[string, string, string]> = [
    // ZWSP preservation
    ["ZWSP internal kept",
      `طلب${ZWSP}المريض إلغاء الحجز`,                                          `طلب${ZWSP}المريض إلغاء الحجز`],
    ["ZWSP wrapped + edge ws trimmed",
      ` \n${ZWSP}طلب المريض${ZWSP}\t `,                                        `${ZWSP}طلب المريض${ZWSP}`],
    ["ZWSP-only reason survives",
      `${ZWSP}${ZWSP}${ZWSP}`,                                                 `${ZWSP}${ZWSP}${ZWSP}`],

    // Internal space runs preserved
    ["double space internal kept",
      `اتصل${SP}${SP}المريض لإلغاء الحجز`,                                     `اتصل${SP}${SP}المريض لإلغاء الحجز`],
    ["internal NBSP kept, edge NBSP stripped",
      `${NBSP}اتصل${NBSP}المريض${NBSP}`,                                       `اتصل${NBSP}المريض`],
    ["internal NBSP run kept",
      `اتصل${NBSP}${NBSP}${NBSP}المريض`,                                       `اتصل${NBSP}${NBSP}${NBSP}المريض`],
    ["mixed NBSP + space internal kept",
      `اتصل${NBSP}${SP}${NBSP}المريض`,                                         `اتصل${NBSP}${SP}${NBSP}المريض`],
    ["internal tab kept, edge tab stripped",
      `\tاتصل\tالمريض\t`,                                                      `اتصل\tالمريض`],
  ...
  try {
    for (const [name, raw, want] of cases) {
      await test(`cancelled + ${name}: audit.reason === expected`, async () => {
        ...
        const { error } = await adminC.rpc("update_appointment_status" as any,
          { _id: id, _status: "cancelled", _reason: trimmed });
    ...
    await test("internal-only spaces + ZWSP is NOT rejected as blank", async () => {
      ...
      const rows = await auditOf(id);
      assert(rows.length === 1 && rows[0].reason === trimmed,
