...
-- 1) CHECK constraint: reason, when present, must be non-empty after normalization
ALTER TABLE public.appointment_audit
  DROP CONSTRAINT IF EXISTS appointment_audit_reason_not_blank;

ALTER TABLE public.appointment_audit
  ADD CONSTRAINT appointment_audit_reason_not_blank
  CHECK (
    reason IS NULL
    OR length(public.normalize_reason(reason)) > 0
  );

-- 2) Rewrite trigger with clear, localized error messages and pre-normalized storage
...
BEGIN
  IF NEW.status IS DISTINCT FROM OLD.status OR NEW.notes IS DISTINCT FROM OLD.notes THEN
    ...
    -- Defensive: if any reason is provided at all, it must not be blank after normalization
